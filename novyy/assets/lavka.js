...
window.TuchaLavka = (function () {
  ...
  function tovar(box) {
    ...
    box.innerHTML =
      '<nav class="kroshki" aria-label="Путь"><a href="' + R + 'lavka/">Лавка</a><span aria-hidden="true">/</span>' +
    ...
    function risovat() {
      ...
      if (!k.length) {
        box.innerHTML = '<div class="lv-pusto lv-pusto-bol"><div class="lv-pusto-il" style="background:' + tint(PO_ID[7] || D.tovary[0]) + '">' + upak(PO_ID[7] || D.tovary[0]) + '</div>' +
          ...
          '<a class="btn" href="' + R + 'lavka/">Открыть Лавку</a></div>';
        return;
      }
      ...
      box.innerHTML = '<div class="kz-g"><div class="kz-sp">' + k.map(stroka).join('') +
        ...
        '<a class="btn btn-bol" href="' + R + 'lavka/zakaz/">Оформить заказ</a>' +
  ...
  var TIPY = { sam: 'Самовывоз со склада', pvz: PS.im, mashina: 'Машиной Тучи', poputka: 'Доставка попуткой', tk: 'Транспортной компанией' };
  function stTip(o) { return STATUSY[o.tip === 'sam' ? 'sam' : o.tip === 'pvz' ? 'pvz' : 'dost']; }
  function pvzImya(id) { var x = (T.PVZ || []).filter(function (p) { return p[0] === id; })[0]; return x ? PS.kratko + ' ' + x[1] : PS.im; }
  ...
  function zakaz(box) {
    ...
    function svodka() {
      return '<p class="kz-mash-h"><b>Ваш заказ</b><small>' + it.poz + ' ' + plural(it.poz, ['позиция', 'позиции', 'позиций']) + ', одна машина</small></p>' +
        ...
        '<div class="kz-k"><span>Итого</span><b>' + rub(vsego()) + '</b></div><p class="muted kz-mel">Что-то не сходится? <a href="' + R + 'kontakty/">Напишите нам</a>, разберёмся до оплаты.</p>';
    }
    ...
    function shag1() {
      ...
      if (z.tip === 'pvz') {
        h += '<fieldset class="pole"><legend>' + PS.kratko + '</legend><div class="vybor">' + (T.PVZ || []).map(function (x) {
      ...
      return h + '<div class="shag-niz"><a class="btn-t" href="' + R + 'lavka/korzina/">Назад в корзину</a><button type="submit" class="btn">Дальше</button></div>';
    }
    function shag2() {
      return '<h2>Кто получает</h2><p class="muted">' + (z.tip === 'pvz' ? PS.p.charAt(0).toUpperCase() + PS.p.slice(1) + ' назовёте код из SMS' : 'Кладовщик спросит имя при выдаче' + (z.tip === 'mashina' ? ', водитель позвонит перед приездом' : '')) + '.</p>' +
    ...
    function gotovo(o) {
      ...
      return '<div class="zk-gotovo"><div class="zk-galka" aria-hidden="true"><svg viewBox="0 0 48 48"><path d="M13 25l7 7 15-16" fill="none" stroke="#fff" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/></svg></div>' +
        ...
        '<div class="cta-pol"><a class="btn" href="' + (T.sessiya() ? R + (T.v2() ? 'v2/' : '') + 'kabinet/#pokupki' : R + 'lavka/pokupki/') + '">Мои покупки</a><a class="btn btn-2" href="' + R + 'lavka/">Вернуться в Лавку</a></div></div>';
    }
  ...
  function vitrina(box) {
    ...
    f.addEventListener('submit', function (e) {
      ...
      f.innerHTML = '<div class="gotovo"><b>Карточка на проверке</b>Пётр Ким посмотрит товар на складе и позвонит ' + kogdaZvonok() +
        ...
        '<p class="cta-pol"><a class="btn btn-2 btn-sm" href="' + R + 'lavka/">Посмотреть Лавку</a>' + (a ? (a.kompaniya ? '<a class="btn btn-sm" href="' + R + (T.v2() ? 'v2/' : '') + 'kabinet/#vitrina">Витрина в кабинете</a>' : '') : '<a class="btn btn-sm" href="' + R + 'start/">Начать работу со складом</a>') + '</p>';
    });
    risovat();
  }
  API.vitrina = vitrina;
