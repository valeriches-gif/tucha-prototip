/* Кабинет второго варианта: собирается из kabinet.js (scratchpad/patch_kab2.py), правки только там.
...
(function () {
  ...
  if (!T.sessiya()) { location.replace(R + 'v2/vhod/'); return; }
  var a = T.akk();
  var STATUSY = ['Мир сохранён', 'Персонаж на связи', 'Готовим договор', 'Договор подписан', 'Товар под тучей'];
  var RAZDELY = [['glavnaya', 'Главная'], ['urovni', 'Уровни'], ['zayavki', 'Заявки'], ['tovar', 'Мой товар'], ['magazin', 'Мой магазин'], ['pokupki', 'Покупки'], ['dokumenty', 'Документы'], ['manager', 'Персонаж'], ['nastroyki', 'Настройки']];
  ...
  function rMenu() {
    menu.innerHTML = RAZDELY.filter(function (r) { return (r[0] !== 'urovni' || mir()) && (r[0] !== 'magazin' || !fl()); }).map(function (r) {
      var zam = (r[0] === 'zayavki' || r[0] === 'dokumenty' || r[0] === 'tovar') && (a.status || 0) < 3;
  ...
  function verh(h) {
    return '<div class="kab-verh"><div><h1 class="kab-h">' + h + '</h1>' + rProfil() + '</div>' +
  ...
  function rSvodka() {
    ...
    var ya = [
      ...
      ['Мой товар', st >= 3 ? tovary().length + ' позиции' : 'пока нет', st >= 3 ? 'остатки и места в разделе «Мой товар»' : 'после первой приёмки'],
  ...
  var DEYSTVIYA = [['Заявить приёмку', 'привезти товар на склад', 'zayavki', 'priemka'], ['Заказать отгрузку', 'на маркетплейс или самовывоз', 'zayavki', 'otgruzka'],
    ['Заказать доставку', 'отвезти по адресу', 'zayavki', 'dostavka'], ['Карточка на Витрине', 'продавать товар в Лавке', 'magazin', 'nova']];
  function rDeystviya() {
    var zakr = (a.status || 0) < 3;
    return '<div class="kp-deystviya">' + DEYSTVIYA.map(function (d) {
      if (d[2] === 'magazin' && fl()) return '';
      return '<button type="button" class="kp-d" data-idi="' + d[2] + '" data-forma="' + d[3] + '"><b>' + d[0] + '</b><span>' + (zakr && d[2] !== 'magazin' ? 'после договора' : d[1]) + '</span></button>';
    }).join('') + '</div>';
  }
  ...
  /* ===== второй вариант: профиль, заявки, товар, магазин ===== */
  function rProfil() {
    var est = a.lichnyy || fl(), komp = !!a.kompaniya && a.tip !== 'fl' || a.bylaKompaniya;
    var imyaP = fl() ? 'Частное лицо · ' + esc(a.imya || a.tel) : esc(a.kompaniya) + ' · ИНН ' + esc(a.inn);
    var drugoy = fl() ? (a.bylaKompaniya ? 'Перейти в профиль компании' : '') : (a.lichnyy ? 'Перейти в личный профиль' : 'Добавить личный профиль');
    return '<p class="muted k2-profil"><span>' + imyaP + '</span>' + (drugoy ? '<button type="button" class="btn-t" data-profil>' + drugoy + '</button>' : '') + '</p>';
  }
  function smenaProfilya() {
    if (fl()) { a.tip = 'ul'; a.bylaKompaniya = false; T.toast('Профиль компании'); }
    else { a.bylaKompaniya = true; a.lichnyy = true; a.tip = 'fl'; T.toast('Личный профиль: покупки и хранение на вас лично'); }
    sohr(); pokaz(razdel === 'magazin' ? 'glavnaya' : razdel);
  }
  var TOVAR_DEMO = [
    { art: 'SIR-1L', name: 'Сироп ванильный, 1 л', ost: 312, ed: 'короб', mesto: 'A-02-07', foto: '14 сент.' },
    { art: 'STAK-12', name: 'Стаканы бумажные 350 мл', ost: 86, ed: 'короб', mesto: 'A-03-11', foto: '12 сент.' },
    { art: 'SALF-3', name: 'Салфетки двухслойные', ost: 40, ed: 'паллета', mesto: 'B-01-02', foto: '9 сент.' },
    { art: 'KOF-1K', name: 'Кофе в зёрнах, 1 кг', ost: 128, ed: 'короб', mesto: 'A-02-09', foto: '5 сент.' }];
  function tovary() { return a.tovary || TOVAR_DEMO; }
  function zayavki() { return a.zayavki || []; }
  var ZSTATUS = { priemka: ['Принята', 'Машина в пути', 'Разгружаем', 'Товар под тучей'], otgruzka: ['Принята', 'Собираем', 'Готова к отгрузке', 'Отгружена'],
    dostavka: ['Принята', 'Собираем', 'В пути', 'Доставлена'] };
  var ZNAZV = { priemka: 'Приёмка', otgruzka: 'Отгрузка', dostavka: 'Доставка' };
  var forma = 'priemka', predTovar = '';
  function zavtra() { var d = new Date(Date.now() + 864e5); return d.toISOString().slice(0, 10); }
  ...
  function okna(id) { return '<select id="' + id + '" name="okno"><option>9:00-12:00</option><option>12:00-15:00</option><option>15:00-18:00</option></select>'; }
  function vyborTovara() {
    return '<select id="z-tovar" name="tovar">' + tovary().map(function (t) {
      return '<option value="' + t.art + '"' + (t.art === predTovar ? ' selected' : '') + '>' + esc(t.name) + ' · ' + t.ost + ' ' + t.ed + '</option>';
    }).join('') + '</select>';
  }
  function rForma() {
    var h = '<div class="k2-tabs" role="tablist">' + ['priemka', 'otgruzka', 'dostavka'].map(function (v) {
      return '<button type="button" role="tab" data-forma-tab="' + v + '" aria-selected="' + (v === forma) + '"' + (v === forma ? ' class="on"' : '') + '>' + ZNAZV[v] + '</button>';
    }).join('') + '</div><form class="k2-f" novalidate data-zayavka="' + forma + '">';
    if (forma === 'priemka') {
      h += '<div class="zk-dva">' + pole('z-data', 'Когда привезёте', '<input id="z-data" name="data" type="date" value="' + zavtra() + '">') + pole('z-okno', 'Время', okna('z-okno')) + '</div>' +
        '<fieldset class="pole"><legend>Что везёте</legend><div class="vybor"><label><input type="radio" name="chto" value="паллет" checked><span>Паллеты</span></label>' +
        '<label><input type="radio" name="chto" value="коробов"><span>Короба</span></label></div></fieldset>' +
        '<div class="zk-dva">' + pole('z-kol', 'Сколько', '<input id="z-kol" name="kol" type="number" min="1" value="6" inputmode="numeric">') +
        pole('z-mash', 'Номер машины <span class="nb">если знаете</span>', '<input id="z-mash" name="mash" type="text" placeholder="А123ВС 777">') + '</div>' +
        '<p class="muted k2-mel">Каждую паллету пересчитаем и сфотографируем, отчёт придёт в мессенджер.</p>';
    } else if (forma === 'otgruzka') {
      h += pole('z-tovar', 'Товар', vyborTovara()) +
        '<div class="zk-dva">' + pole('z-kol', 'Сколько', '<input id="z-kol" name="kol" type="number" min="1" value="10" inputmode="numeric">') +
        pole('z-data', 'Когда', '<input id="z-data" name="data" type="date" value="' + zavtra() + '">') + '</div>' +
        '<fieldset class="pole"><legend>Куда</legend><div class="vybor">' + ['Wildberries FBS', 'Ozon FBS', 'Самовывоз', 'Транспортная компания'].map(function (x, i) {
          return '<label><input type="radio" name="kuda" value="' + x + '"' + (i ? '' : ' checked') + '><span>' + x + '</span></label>';
        }).join('') + '</div></fieldset>';
    } else {
      h += pole('z-tovar', 'Товар', vyborTovara()) +
        '<div class="zk-dva">' + pole('z-kol', 'Сколько', '<input id="z-kol" name="kol" type="number" min="1" value="4" inputmode="numeric">') +
        pole('z-data', 'Когда', '<input id="z-data" name="data" type="date" value="' + zavtra() + '">') + '</div>' +
        pole('z-adres', 'Адрес', '<input id="z-adres" name="adres" type="text" placeholder="Город, улица, дом">') + pole('z-okno', 'Окно доставки', okna('z-okno')) +
        '<p class="muted k2-mel">Стоимость пришлём до выезда: считаем по объёму и адресу.</p>';
    }
    return h + '<button class="btn" type="submit">Отправить заявку</button></form>';
  }
  function rZayavki() {
    var sp = zayavki();
    if (!sp.length) return '<p class="pusto">Заявок пока нет. Первая появится здесь сразу после отправки.</p>';
    return sp.slice().reverse().map(function (z) {
      var st = ZSTATUS[z.vid];
      return '<div class="k2-z"><div class="k2-z-v"><b>' + ZNAZV[z.vid] + ' № ' + z.n + '</b><span class="muted">' + esc(z.opis) + '</span></div>' +
        '<ol class="k2-st">' + st.map(function (s, i) { return '<li class="' + (i < z.st ? 'done' : i === z.st ? 'on' : '') + '">' + s + '</li>'; }).join('') + '</ol>' +
        (z.st < st.length - 1 ? '<button type="button" class="btn-t" data-z-dalee="' + z.n + '">Демо: следующий шаг</button>' : '') + '</div>';
    }).join('');
  }
  function otpravitZayavku(f) {
    var fd = new FormData(f), v = f.getAttribute('data-zayavka'), kol = +fd.get('kol') || 0;
    if (kol < 1) { T.toast('Укажите количество'); f.querySelector('[name=kol]').focus(); return; }
    var t = tovary().filter(function (x) { return x.art === fd.get('tovar'); })[0];
    if (v === 'dostavka' && !String(fd.get('adres') || '').trim()) { T.toast('Укажите адрес доставки'); f.querySelector('[name=adres]').focus(); return; }
    var opis = v === 'priemka' ? kol + ' ' + fd.get('chto') + ', ' + datTxt(fd.get('data')) + ', ' + fd.get('okno') :
      v === 'otgruzka' ? (t ? t.name : '') + ', ' + kol + ' ' + (t ? t.ed : '') + ' → ' + fd.get('kuda') + ', ' + datTxt(fd.get('data')) :
      (t ? t.name : '') + ', ' + kol + ' ' + (t ? t.ed : '') + ' → ' + fd.get('adres') + ', ' + datTxt(fd.get('data')) + ', ' + fd.get('okno');
    var sp = zayavki(), n = 4180 + sp.length + 1;
    sp.push({ n: n, vid: v, opis: opis, st: 0, t: Date.now() }); a.zayavki = sp; sohr();
    predTovar = ''; pokaz('zayavki');
    T.toast(ZNAZV[v] + ' № ' + n + ' принята. ' + (mir() ? imya() + ' подтвердит' : 'Менеджер подтвердит') + ' в течение часа');
  }
  function rTovar() {
    return '<div class="kart k2-tabl-k"><table class="k2-tabl"><thead><tr><th>Товар</th><th>Остаток</th><th>Место</th><th>Приёмка</th><th></th></tr></thead><tbody>' +
      tovary().map(function (t) {
        return '<tr><td><b>' + esc(t.name) + '</b><span class="muted">' + t.art + '</span></td><td>' + t.ost + ' ' + t.ed + '</td><td>' + t.mesto + '</td>' +
          '<td>' + t.foto + ', фото и пломба</td><td class="k2-kn"><button type="button" class="btn btn-2 btn-sm" data-otgruzit="' + t.art + '">Отгрузить</button>' +
          (fl() ? '' : '<button type="button" class="btn-t" data-na-vitrinu="' + t.art + '">На Витрину</button>') + '</td></tr>';
      }).join('') + '</tbody></table></div>';
  }
  function kartyVitriny() { return T.st.get('tucha.vitrina') || []; }
  function rMagazin() {
    var sp = kartyVitriny();
    var h = '<div class="kp-svodka k2-mag"><div><span class="kp-z">Карточек</span><b>' + sp.length + '</b><span class="muted">' + (sp.length ? 'на проверке и в Лавке' : 'пока нет') + '</span></div>' +
      '<div><span class="kp-z">Выплата</span><b>по пятницам</b><span class="muted">за всё, что уехало за неделю</span></div>' +
      '<div><span class="kp-z">Сборка заказов</span><b>на складе</b><span class="muted">собираем и отгружаем мы</span></div></div>';
    h += '<div class="kart"><div class="razdel-h"><h2 class="h3">Мои карточки</h2><button type="button" class="btn btn-sm" data-nova>Создать карточку</button></div>' +
      (sp.length ? sp.map(function (c, i) {
        var vLavke = Date.now() - (c.t || 0) > 20000 || c.vLavke;
        return '<div class="usl-str"><span><b>' + esc(c.name) + '</b><br><span class="muted">' + esc(c.cat) + ' · ' + c.one + ' ₽ штучно, ' + c.box + ' ₽ коробом, ' + c.pal + ' ₽ паллетой</span></span>' +
          '<span class="k2-chip' + (vLavke ? ' ok' : '') + '">' + (vLavke ? 'В Лавке' : 'На проверке') + '</span></div>';
      }).join('') : '<p class="pusto">Карточек пока нет. Создайте первую: товар уже лежит на складе, осталось поставить цены.</p>') + '</div>';
    return h;
  }
  ...
  var RENDER = {
    glavnaya: function () {
      ...
      if (mir()) {
        ...
        h += rHud() + rDeystviya() + '<div class="kab-mir"><div class="mir-scena kab-scena"><svg id="kabScena" role="img" aria-label="Ваш мир: постройка под тучей"></svg></div>' +
          '<div class="kab-kol">' + (verhUr ? rZadanie() : '') + rStatus() + '</div></div>' + rKartaUr() +
          '<div class="setka s2 kab-niz">' + rManager() + rDost() + '</div>';
      } else {
        h += rSvodka() + rDeystviya() + '<div class="kp-g"><div>' + rStatus() + ((a.status || 0) >= 3 ? '' : rPrigotovit()) + '</div><div>' + rManager() + rSkidka() + '</div></div>' +
    ...
    zayavki: function () {
      var st = a.status || 0;
      if (st >= 3) return verh('Заявки') + '<div class="k2-zg"><div class="kart">' + rForma() + '</div><div class="kart"><h2 class="h3">Мои заявки</h2>' + rZayavki() + '</div></div>';
    ...
    tovar: function () {
      if ((a.status || 0) < 3) return verh('Мой товар') + '<div class="zamok"><b>Откроется после договора</b><p>Здесь будут остатки, места на складе и фото каждой приёмки.</p></div>';
      return verh('Мой товар') + '<p class="muted">Остатки обновляются после каждой приёмки и отгрузки.</p>' + rTovar();
    },
    magazin: function () {
      if (fl()) return RENDER.glavnaya();
      if (novaya) return verh('Новая карточка') + '<p><button type="button" class="btn-t" data-k-mag>← Мои карточки</button></p><div data-vitrina-kab></div>';
      return verh('Мой магазин') + '<p class="muted">Витрина: ваш товар со склада продаётся в Лавке. Хранение не меняется, сборку делаем мы.</p>' + rMagazin();
    },
    dokumenty: function () {
      var st = a.status || 0;
    ...
    nastroyki: function () {
      return verh('Настройки') + '<div class="setka s2">' +
        ...
        '<div class="kart"><h2 class="h3">Услуги</h2><p class="muted">' + (uslugi().map(function (x) { return S.BLOKI[x].ob; }).join(', ') || 'не выбраны') + '</p>' +
        (mir() ? '<p><a class="btn btn-2 btn-sm" href="' + R + 'start/mir/?dostroit=1">Изменить постройку</a></p>' : '<p><button type="button" class="btn btn-2 btn-sm" data-idi="uslugi">Добавить услугу</button></p>') + '</div>' +
  ...
  function pokaz(r) {
    ...
    var vk = box.querySelector('[data-vitrina-kab]');
    if (vk && window.TuchaLavka) {
      TuchaLavka.vitrina(vk);
      if (predTovar) { var tv = tovary().filter(function (x) { return x.art === predTovar; })[0], ni = vk.querySelector('#vt-name'); if (tv && ni) { ni.value = tv.name; ni.dispatchEvent(new Event('input', { bubbles: true })); } }
    }
  ...
  menu.addEventListener('click', function (e) {
    ...
    if (b) { novyy = null; novaya = false; pokaz(b.dataset.r); box.querySelector('h1').focus({ preventScroll: true }); }
  });
  box.addEventListener('click', function (e) {
    ...
    var d = t.dataset;
    if (d.idi) {
      if (d.forma === 'nova') { novaya = true; predTovar = ''; }
      else if (d.forma) { forma = d.forma; }
      pokaz(d.idi); box.querySelector('h1').focus({ preventScroll: true });
    }
    else if (d.formaTab) { forma = d.formaTab; pokaz('zayavki'); }
    else if (d.zDalee) { zayavki().forEach(function (z) { if (String(z.n) === d.zDalee) z.st = Math.min(ZSTATUS[z.vid].length - 1, z.st + 1); }); sohr(); pokaz('zayavki'); T.toast('Статус заявки обновлён. Отправили вам сообщение'); }
    else if (d.otgruzit) { forma = 'otgruzka'; predTovar = d.otgruzit; pokaz('zayavki'); }
    else if (d.naVitrinu) { novaya = true; predTovar = d.naVitrinu; pokaz('magazin'); }
    else if ('nova' in d) { novaya = true; predTovar = ''; pokaz('magazin'); }
    else if ('kMag' in d) { novaya = false; pokaz('magazin'); }
    else if ('profil' in d) smenaProfilya();
  ...
  box.addEventListener('submit', function (e) {
    var f = e.target.closest('form[data-zayavka]');
    if (f) { e.preventDefault(); otpravitZayavku(f); }
  });
