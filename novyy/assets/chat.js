/* Помощник на сайте — бот, не живой человек. Отвечает сразу, днём и ночью,
   по базе частых вопросов; чего не знает — предлагает передать менеджеру.
...
window.TuchaChat = (function () {
  ...
  var PRIVET = 'Здравствуйте! Я помощник Тучи, бот, не живой человек. Отвечаю сразу, днём и ночью. ' +
    'Сложный вопрос передам менеджеру.';
  var BAZA = [
    { k: ['минимал', 'одной паллет', 'одну паллет', 'от скольк', 'мало товар', 'небольш'], a: 'Минимального объёма нет, храним от одной паллеты.' },
    { k: ['цен', 'стоит', 'стоимост', 'тариф', 'прайс', 'почём', 'дорого'], a: 'Хранение: от 16,42 ₽ за паллето-место в сутки, с НДС. При резервации мест на срок, скидка. Обработку, доставку и таможню считает менеджер. <a href="~/hranenie/#kalk">Посчитать хранение</a>' },
    ...
    { k: ['часы', 'график', 'во сколько', 'выходн', 'суббот', 'воскрес', 'время работ', 'режим работ'], a: 'Склад и менеджеры работают пн-пт с 9:00 до 18:00. Я на связи всегда.' },
    ...
    { k: ['витрин', 'продавать', 'выставить', 'продавц'], a: 'Витрина: ваш товар в Лавке, прямо с полки, где он хранится. Сборку и отгрузку делаем мы, выплата по пятницам. <a href="~/vitrina/">Собрать карточку</a>' },
    { k: ['лавк', 'купить', 'покупател', 'корзин', 'заказ товар'], a: 'Лавка работает: товар прямо со склада, от штуки до паллеты, забрать можно в день заказа. <a href="~/lavka/">Открыть каталог</a>' },
    { k: ['физлиц', 'физ лиц', 'частн', 'личные вещ'], a: 'Хранение: для компаний и ИП. А покупать в <a href="~/lavka/">Лавке</a> может любой: и компания, и частное лицо.' },
    { k: ['резерв', 'займи', 'брониров', 'скидк'], a: '«Займи место под тучей», резервация мест на срок со скидкой на хранение. Сетку скидок назовёт менеджер.' },
    ...
    { k: ['человек', 'менеджер', 'оператор', 'живой', 'позвон', 'связат'], a: 'Позову менеджера: звоните <a href="tel:+74956658242">+7 (495) 665-82-42</a> или <a href="https://t.me/tucha_ml">напишите в Telegram</a>. Могу и передать ваш вопрос, ответят в рабочее время.', peredat: true },
  ...
  function sozdat(box, o) {
    ...
    function sprosit(v) {
      ...
      setTimeout(function () {
        ...
        if (x) {
          ...
          soob('bot', 'Пока не знаю ответа. Передать вопрос менеджеру? Он ответит в рабочее время, пн-пт с 9:00 до 18:00.');
          knopkaPeredat(v);
        }
      }, 550);
    }
    function knopkaPeredat(v) {
      ...
      b.innerHTML = '<button type="button" class="btn btn-2 btn-sm">Передать менеджеру</button>';
      b.querySelector('button').onclick = function () {
        b.remove();
        soob('bot', 'Передал. Менеджер ответит в рабочее время. Если срочно: <a href="tel:+74956658242">+7 (495) 665-82-42</a>.');
        T.goal('chat_menedzher');
        T.toast('Вопрос передан менеджеру (демо)');
      };
      lenta.appendChild(b);
      lenta.scrollTop = lenta.scrollHeight;
    }
