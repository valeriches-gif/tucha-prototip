...
(function (I) {
  ...
  function vitrinaRis(g, t, ctx) {
    var z = H1 + H2, h = H3 * t;
    boks(g, 2.5, 2.25, 2, 1, z, h, C.vitr, { 'fill-opacity': .88 });
    if (t < .92) return;
    var L = granY(g, 2.5, 2.25, 2, 1, z, h);
    ...
    var R = granX(g, 2.5, 2.25, 2, 1, z, h);
    el('rect', { x: 3, y: 6, width: 30, height: 12, fill: BALKA }, R);
    txt(R, 18, 14.8, 'ВИТРИНА', { 'font-family': SK, 'font-size': 7.4, 'font-weight': 900, fill: CHERN, 'letter-spacing': '.05em' });
    var v = P(4.5, 2.25, z + h), zv = el('g', { transform: 'translate(' + v[0].toFixed(1) + ',' + (v[1] - 10).toFixed(1) + ')' }, g);
