...
window.Tucha = (function () {
  ...
  /* пункт выдачи везде называется Станция Тучи; в «Всё просто» и у гостей рядом поясняем, что это пункт выдачи */
  function pvzSlova() {
    var a = akk();
    var mir = a && a.vid === 'mir';
    return { im: mir ? 'Станция Тучи' : 'Станция Тучи, пункт выдачи', v: 'на Станцию Тучи', p: 'на Станции Тучи', kratko: 'Станция Тучи',
      edet: 'Едет на Станцию', zhdet: 'Ждёт на Станции', poyasn: mir ? '' : 'Наш пункт выдачи у метро. ' };
  }
  ...
  function ozhivitProvodnika() {
    var kartinki = document.querySelectorAll('.hero-malchik img[data-pv-net]');
    if (!kartinki.length) return;
    var tiho = window.matchMedia && matchMedia('(prefers-reduced-motion: reduce)').matches;
    [].forEach.call(kartinki, function (img) {
      var baza = img.getAttribute('src').replace('geroy.webp', '');
      var put = function (n) { return baza + n + '.png'; };
      var sloy = function (klass, n, data) {
        return '<img class="' + klass + '"' + (data ? ' ' + data : '') + ' src="' + put(n) + '" alt="" width="272" height="504">';
      };
